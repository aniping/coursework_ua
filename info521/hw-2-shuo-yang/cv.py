...
def run_cv( K, maxorder, x, t, randomize_data=False, title='CV' ):
    ...
    for p in range(maxorder + 1):
        ...
        for fold in range(K):
            ...
            ### YOUR CODE HERE ###
            # w = None  # Calculate w vector (as an numpy.array)
            w = fitpoly(trainX, traint, p)  # Calculate w vector (as an numpy.array)
    ...
    plot_cv_results(log_mean_train_loss, log_mean_cv_loss, log_scale_p=True)
...
def run_demo():
    ...
    x = data[:, 0] # get x vector
    t = data[:, 1] # get t vector

    K = 10 # 10-fold

    run_cv( K, 7, x, t, randomize_data=True, title='{0}-fold CV'.format(K) )

    K = data.shape[0] # LOOCV
    run_cv( K, 7, x, t, randomize_data=True, title='{0}-fold CV'.format(K) )
