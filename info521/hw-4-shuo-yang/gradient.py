...
def compute_gradient_numerical_estimate(J, theta, epsilon=0.0001):
    ...
    # iterate through each parameter in theta, vary its value by +-epsilon to numerically
    # compute the gradient for each parameter
    for i in range(theta.shape[0]):
        theta_plus = numpy.array(theta, dtype=numpy.float64)
        theta_plus[i] = theta[i] + epsilon
        theta_minus = numpy.array(theta, dtype=numpy.float64)
        theta_minus[i] = theta[i] - epsilon

        gradient[i] = (J(theta_plus)[0] - J(theta_minus)[0]) / (2 * epsilon)
