...
/*static short find_empty_slot(class HFPage *p)
{
	if (p->slotCnt == 0)
		return 0;
...
static int check_valid_rid(class HFPage *p, RID rid)
{
	if (rid.slotNo < 0 || rid.slotNo >= p->slotCnt)
		return -1;
		
	//slot_t *slot_ptr = (slot_t *)(data + (rid.slotNo - 1) * sizeof(slot_t));
	slot_t *slot_ptr = &(p->slot[rid.slotNo]);
